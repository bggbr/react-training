	...
	return produce(_state, (state) => {
		switch (action.type) {
			...
			case 'change-cookingTime': {
				...
				state.menu[idx].history.unshift({
					date: dateFormat(new Date()),
					type: 'time',
					lastVal: state.menu[idx].cookingTime,
					updateVal: parseInt(time),
				});
			...
			case 'change-price': {
				...
				state.menu[idx].history.unshift({
					date: dateFormat(new Date()),
					type: 'price',
					lastVal: state.menu[idx].price,
					updateVal: parseInt(price),
				});
			...
			case 'change-category': {
				...
				state.menu[idx].history.unshift({
					date: dateFormat(new Date()),
					type: 'category',
					lastVal: state.menu[idx].category,
					updateVal: category,
				});
